...
namespace FiveWordFinderConsole.Services
{
	public class ConsoleProgress : IDisposable, IProgress<double>
	{
		...
		public void Report(double value)
		{
			...
			Interlocked.Exchange(ref currentMessage, string.Empty);
		}
		...
		public void Report(double value, string message)
		{
			...
			value = Math.Max(0, Math.Min(1, value));
			Interlocked.Exchange(ref currentProgress, value);
			Interlocked.Exchange(ref currentMessage, message);
		}
		...
		private void TimerHandler(object? state)
		{
			lock (timer)
			{
				...
				text = string.Format("{0} {1}", text, animation[animationIndex++ % animation.Length]);
				UpdateText(text);
